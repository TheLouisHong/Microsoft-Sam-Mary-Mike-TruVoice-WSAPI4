...
{
DWORD    dwSizeNeeded = dwNumElems * sizeof(CLLISTELEM);
PVOID    pTemp;
...
if (paElems) {
   ...
   paElems = (PCLLISTELEM) pTemp;
   dwBufSize = dwSizeNeeded;
   return TRUE;   // done
   };

// else its a job for malloc
paElems = (PCLLISTELEM) malloc (dwSizeNeeded);
if (paElems)
   dwBufSize = dwSizeNeeded;
return (paElems != NULL);
}
...
memmove (paElems + (1+dwElemNum), paElems + dwElemNum,
   (dwNumElems - dwElemNum) * sizeof(CLLISTELEM));
dwNumElems++;
...
memmove (paElems + dwElemNum, paElems + (dwElemNum+1),
   (dwNumElems - dwElemNum - 1) * sizeof(CLLISTELEM));
dwNumElems--;
return TRUE;
}
